...
(function aiPlannerModule(root) {
  ...
  function callAzure(messages, maxTokens, callType) {
    if (!isConfigured()) {
    ...
    return fetch(buildUrl(), {
      ...
      /* Track API cost if CSCostTracker available */
      if (window.CSCostTracker && data.usage) {
        try {
          var inputTokens = Number(data.usage.prompt_tokens || 0);
          var outputTokens = Number(data.usage.completion_tokens || 0);
          var costInfo = window.CSCostTracker.trackCall(
            callType || "general",
            inputTokens,
            outputTokens
          );
          ...
          if (text) {
            ...
            text += "\n\n<!-- Cost tracked: $" + costInfo.callCost.toFixed(4) + " -->";
          }
        } catch (_e) {
          console.warn("Cost tracking error:", _e);
        }
      }
